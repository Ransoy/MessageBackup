...
$(function() {
	...
	$('#btn_submit_message').click( function(e) {
		...
		if (window.FormData == undefined) {
			...
			if (!touchStart) {
				touchStart = true;
				fileInput = (imgData == null) ? new FormData($('#message-form')[0]) : imgData;
	...
	$('#btn_photo').on('click', function(e){
		...
		if ((/iPhone|iPod|iPad/.test(window.navigator.platform)) || isNexus.test(navigator.userAgent)) {
			if (!isDisableSend) {
				$('#file').trigger("click");
			}
		} else {
	...
	// message list edit
	$(document).on('click', '.message_list--edit .message_list_item .thumb,.message_list_item .col_left ,.message_list_item .col_center , .message_list--edit .message_list_item .message_data', function(e){
		e.preventDefault();
		e.stopPropagation();
