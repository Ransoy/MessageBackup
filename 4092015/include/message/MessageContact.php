...
class MessageContact{
	...
	function initiateProperties() {
		...
		$timeStart = $this->db->sqlLogging('MessageContact', 'initiateProperties');
	...
	function create(){
		...
		if($result){
			...
			$timeStart = $this->db->sqlLoggingInsert('MessageContact', 'create', $data);
	...
	function getContact() {
		$this->db->select('*');
		$this->db->from('message_contact');
		$this->db->where('BINARY(user_id)',"BINARY('".$this->getUserId()."')");
		$this->db->where('BINARY(performer_id)',"BINARY('".$this->getPerformerId()."')");
		$timeStart = $this->db->sqlLogging('MessageContact', 'getContact');
		$result = $this->db->get_row();
		$this->db->sqlLoggingTime($timeStart);
		return $result;
	}
	...
	function block($id, $byType){
		...
		$timeStart = $this->db->sqlLoggingInsert('MessageContact', 'block', $data);
		$result = $this->db->insert($data);
		$this->db->sqlLoggingTime($timeStart);
		return $result;
	...
	function unblock($id,$type,$fromId,$toId){
		...
		if($fromId !=null && $toId != null){
			$timeStart = $this->db->sqlLoggingUpdate('MessageContact', 'unblock', $data,'message_contact_block');
	...
	function updateMessage($fromId, $toId){
		...
		$timeStart = $this->db->sqlLoggingUpdate('MessageContact', 'updateMessage', $data,'message');
	...
	function fave($id, $byType){
		...
		$timeStart = $this->db->sqlLoggingInsert('MessageContact', 'fave', $data);
		$result = $this->db->insert($data);
		$this->db->sqlLoggingTime($timeStart);
		return $result;
	...
	function unfave($id){
		...
		$this->db->where('contact_id',$id);
		$timeStart = $this->db->sqlLoggingDelete('MessageContact', 'unfave');
		$result = $this->db->delete();
		$this->db->sqlLoggingTime($timeStart);
		return $result;
	...
	function isBlockedBy($id, $byType){
		...
		$timeStart = $this->db->sqlLogging('MessageContact', 'isBlockedBy');
		$result = $this->db->get_row();
		$this->db->sqlLoggingTime($timeStart);
	...
	function isFaveBy($id, $byType){
		...
		$timeStart = $this->db->sqlLogging('MessageContact', 'isFaveBy');
		$result = $this->db->get_row();
		$this->db->sqlLoggingTime($timeStart);
		return ($result)? true : false ;
	}
