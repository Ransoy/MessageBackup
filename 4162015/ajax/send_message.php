...
if (
	...
	if ($hasBody || $hasImage) {
		if ($hasBody) { // Submitted by Message Text
			$decodeMsg = urldecode($_POST['message_body']);
			$msg = unicode_decode($decodeMsg);
			$messageBody = mb_convert_encoding($msg, 'HTML-ENTITIES', 'UTF-8'); //change to &bull
			$messageBody = htmlentity_decode($messageBody); //from entities to japanese
			
			$messageBody =  htmlentity_encode($messageBody);
			//exit();
			if (trim(mb_trim($msg)) != '') {
	...
	if ($messageContactHelper->checkInvalidStatus($toType, $toId)) {
	...
	if ($messageResult == '') {
		...
		if ($toType == 3 || $messageContact->isBlockedBy($contactId, $toType)) {
			$messageResult = "相手のご都合によりメッセージを送ることができません。";
			//$message->setIsSent(1);
		} else {
			$message->setIsSent(1);
			$message->setFromType($fromType);
			...
			if ($result != null) {
				...
				if ($hasBody) {
					//mb_language('ja');
					$forwardMsg = mb_convert_encoding($decodeMsg, 'EUC-JP', 'UTF-8');
					$message->check_ng_word($forwardMsg);
					//$forwardMsg = htmlentity_decode($messageBody);
				}
				$message->tenso_check($toId, $parm, $fromId, $fromType, $forwardMsg, $hasImage);
...
function htmlentity_decode($str) {
	$output = preg_replace_callback("/(&#[0-9]+;)/", 'replace_htmlentites_escape', $str);
	return $output;
}

function replace_htmlentites_escape($m) {
	//mb_language('ja');
	$val = mb_convert_encoding($m[1], "EUC-JP", "HTML-ENTITIES");
	if ($val != '?') {
		return $val;
	} else {
		return $m[1];
...
function htmlentity_encode($str) {
	$replace = array('&lt;', '&gt;'); 
	$find = array('<', '>');  
