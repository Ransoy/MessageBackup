...
$(function(){
	...
	$('.btn_blocklist').on(event, function(e){
		...
		if(contact_id != 0 || contact_id != ''){
			...
			alert('パフォーマーを連絡先に追加しなかったので拒否できません');
		}
	...
	$('#btn_submit_message').on(event, function(e) {
		if (touchStart) {
			...
			if (window.FormData == undefined) {
				...
				if (hasImage && ext != 'image/gif' && isIos != true) {
...
function createMessage(body, fData, fileName) {
	...
	if (fData == '') {
		...
		if (imgData == null) {
			formData.append(fileName, fData);
		}
		formData.append('message_body', body);
	} else {
		if (imgData == null) {
...
function checkNotifyDisp(){
	...
	if($('.notice_msg').is(':visible')) {
	    ...
	    $('.message_wrap .message_detail_area').css({'padding-bottom':hh_notify + 'px'});
		 ...
		 $('.message_wrap .message_detail_area').css({'padding':'0 0 2rem'});
	}
}
...
/**webview app**/
function newDataURItoBlob(dataURI) {
