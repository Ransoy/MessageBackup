	...
	if($contact['data']){
		...
		foreach($contact['data'] as $row) {
			...
			if($row->prof_open_flg == 0 ){
				$flag = '<a class="btn_profile private" href="javascript:void(0);"><img src="/img/spacer.gif" style="background-image: url(/imgs/member/320x240/'.$row->img.')" /></a>';
			}else{
				$flag = '<a class="btn_profile" rel="leanModal" data-type="iframe" data-width="584" data-height="446" href="../male_profile.php?hash='.$row->hash.'"><img src="/img/spacer.gif" style="background-image: url(/imgs/member/320x240/'.$row->img.')" /></a>';
			}
