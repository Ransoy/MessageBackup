...
class Message {
	...
	function uploadImage() {
		...
		if (!empty($image)) {
			...
			$fileProcess =  ($ext == 'jpeg' || $ext == 'jpg' || $ext == 'JPEG') ? $this->fixRotateImage($fileSource) : $fileSource;
			
			if ($fileProcess != $fileSource) {
				ImageJPEG($fileProcess, $fileName, 90);
			} else {
...
// 			print_r($femStat) ;
			...
			/*
			 * check mobile view
			 * getting the value of point settings
			 */
			if(isSmartPhone()){
				$sendPoint = $this->getPointSettings($arrSP[$type][$stat]);
				$recievePoint = $this->getPointSettings($arrSP[$type][$stat]+$pointIndex);
				$money = $this->getPointSettings($arrSP[$type][$stat]+$yenIndex);
				
			}else{
				$sendPoint = $this->getPointSettings($arrPC[$type][$stat]);
				$recievePoint = $this->getPointSettings($arrPC[$type][$stat]+$pointIndex);
				$money = $this->getPointSettings($arrPC[$type][$stat]+$yenIndex);
				
			}
	
			$totalMoney = $femaleM + $money['value'];
	...
	function getProfilePoint($table,$id, $pointOnly = ''){
		...
		if($table == 'female_point'){
			...
			$sel =  ($pointOnly != '') ? 'mp.point' : 'm.stat,m.gold_flg,mp.point,mp.point_old';
			$joinT = 'male_member';
			$alias = 'mp';
			$alias1 = 'm';
		}
	...
	function validateImage($src) {
		...
		if ($_FILES[$src]['tmp_name'] == '') {
			if (1 == $_FILES[$src]['error']) {
				$imageResult = '画像イメージがサーバのアップロードサイズの上限を超えています。';
			}
			$imageResult .= '<br/> 画像がありません。';
		}
		if ($_FILES[$src]['size'] == 0) {
			$imageResult .= '<br/>画像のサイズが 0 になる。';
		}
		if ($_FILES[$src]['size'] > 8388608) {
			$imageResult .= '<br/>ファイルサイズの上限を超えています。';
		}
		$size = GetImageSize($_FILES[$src]['tmp_name']);
		if ($size[2] != 1 && $size[2] != 2 && $size[2] != 3) {
			$imageResult .= '<br/>ファイルは画像ではありません。';
		}
		if (!is_uploaded_file($_FILES[$src]['tmp_name'])) {
			$imageResult .= '<br/>イメージをアップロードしません。';
		}
...
EOM;
			...
			p_sendmail('xinmartinez@gmail.com',"お知らせ禁止ワードチェック報告",$str );
