...
class MessageContactHelper{
	...
	function getAllFor($id, $type, $options = null, $offset = 0, $limit = 0) {
        ...
        if(count($options)>1)
            ...
            $this->db->like('lower(nick_name)', strtolower($value) ,'both');        	
	 ...
	 * @param string $funcName - function name to be called set arguments for the func_name ex: 'getAllFavedBy'
	 * @param int $count_row - total numbers of data
	 * @param string $options - set search value
	 * @param int $limit - set limit data to 1 0
	 * @param int $type - 1 = user , 2 = performer :(default = 1)
	 * @param int $pageType - default value 1 for contact_list page
	 * @param return multiple data[object]
	 *
	 */
	function paginateContact($funcName, $countRow, $options, $limit , $type = 1, $pageType = 1){
		...
		if($pageType == 1){
			...
			$q = '&q='.str_replace(' ', '+', $_GET['q']);
			$url = $getview.$q;
			//call function
			$contactResult = $this->$funcName($_SESSION['user_id'], $type, $options, $offset, $limit);
		}else{
			//call function
			$url = (isset($_GET['name']))? "&name=".str_replace(' ','+',$_GET['name']) : '';
			$contactResult = $this->$funcName($options, $offset, $limit, $_SESSION['user_id']);
		}
		...
		if($totalRow > 1){
			...
			if($page > 1){
				$tmpl->assign('prev-href',$prev.$url);
			...
			if ($_GET['page'] > 5){
				$pagination .= "<li><a href='?page=1$url'>1</a><li>";
				...
				for ($counter = $_GET['page'] - 2; $counter <= $totalRow; $counter++){
					if($count < 5){
						if ($counter == $page){
							...
							$pagination .= "<li><a href='?page=$counter$url'>$counter</a></li>";
						}
						$count++;
					}else{
						$pagination.= "<li><span>...</span></li>";
						$pagination.= "<li><a href='?page=$totalRow$url'>$totalRow</a></li>";
						break;
					}
				}
			}else{
			...
			if ($page < $counter - 1){
				$tmpl->assign('next-href',$next.$url);
	...
	function getContactDetail($fromId, $toId, $toType, $fromType, $hash = ''){
		...
		if($toType != 3){
			...
			$this->db->where("BINARY $fromWhere = '$fromId' AND BINARY $toWhere = '$toId'");
	...
	function searchPerformer($nickname, $start, $limit, $userId) {
		$value = addcslashes($nickname, '!...?');
		if($value == '') {
			return;
		}
		$this->db->select('SQL_CALC_FOUND_ROWS c.id, fm.user_id, fp.hash, fp.img, fp.nick_name');
		$this->db->from('female_member as fm');
		$this->db->join('female_profile as fp', 'fm.user_id = fp.user_id', 'INNER');
		$this->db->join('message_contact as c', "c.user_id = '$userId' AND fm.user_id = c.performer_id", 'LEFT');
		$this->db->where("(c.id is NULL OR c.id NOT IN (SELECT contact_id FROM message_contact_block WHERE contact_id = c.id AND block_by_type = 1))");
		$this->db->where('fm.stat <> 9');
		$this->db->where('fm.stat <> 6');
		
		$this->db->like('lower(fp.nick_name)', strtolower($value),'both');
		
		if($limit != 0){
			$this->db->limit($start, $limit);
		}
		$this->db->order_by('fp.nick_name', 'ASC');
		$result = $this->db->results();
		return $result;
	}
	
	function countSearchPerformer(){
