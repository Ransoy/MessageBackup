...
EOM;
	...
	/**
	 * Check if the receipient has a forwarding mail settings
	 * @param String $to_user
	 * @param String $parm
	 * @param String $from
	 * @param int $from_type
	 * @param String $body
	 * @param boolean $hasImage (0 , 1)
	 */
	function tenso_check($to_user,$parm, $from, $from_type, $body, $hasImage){
		global $ownerCd;
		...
		if($row['tenso_mail1'] == 0){
			if ($from_type == 1) {
				...
				if($tenso_flg == 1){
					//転送先1へメール送信
					$this->mail_send($row['mail'], $from, 1, "", $sender, $row['nick_name'], $body, $hasImage);
				}
				...
				if($tensoFlg == 1){
					$this->mail_send2($row['mail'], $from, 1, $parm, $sender, $row['nick_name'],$row['auth_type'], $body, $hasImage);
				}
		...
		if ($from_type == 2) {
			if($row['mb_tenso_mail_stat'] == 0){
				...
				/*		if($row[1] == "1"){
				 ...
				 $tensoFlg = 0;
					...
					*/
				if($tensoFlg == 1){
					$this->mail_send2($row['tenso_mail2'], $from, 2, $parm, $sender, $row['nick_name'],$row['auth_type'], $body, $hasImage);
				}
			}
			...
			if($row['tenso_mail2'] != ""){ 
				if(!($kyohi3 == "0" && $kyohi4 == "24")){
					...
					if($tenso_flg == 1){
						//転送先2へメール送信
						$this->mail_send($row['tenso_mail2'], $from, 2, $parm, $sender, $row['nick_name'], $body, $hasImage);
					}
				}
			}
		}	
	...
	//--------------------------------------
	//転送メール送信
	function mail_send($tenso_mail, $from, $mail_flg, $parm, $sender, $to_nick, $body, $hasImage) {
		global $mail_id;
		$title = ($hasImage) ? "【マシェリ】　{$from}さんからの新着画像メッセージです。" : "【マシェリ】{$from}様から新着メッセージが届きました。";
		//転送メール１への送信
		if($mail_flg == 1){
...
{$from}様、から新着画像メッセージが届いておりますのでお知らせ致します。
	
━メッセージ内容━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

※画像はサイト内のメッセージボックスをご確認ください。	
	
※メッセージ返信は「コチラ」
　　→　http://www.macherie.tv/performer/message/inbox.php
	
※このメールに返信しても会員様へメールは届きません。
　返信される場合はマシェリメッセージBOXより返信をお願い致します。
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
{$from}様、からメッセージが届いておりますのでお知らせ致します。
...
EOM;
	...
	//----------------------------------------------------------
	// 転送メール送信
	function mail_send2($tenso_mail, $from, $mail_flg, $param, $sender, $to_nick, $auth, $body, $hasImage){
		global $mail_id;
		$title = ($hasImage) ? "マシェリ　{$from}さんからの新着画像メッセージです。" : "マシェリ　{$from}さんからの新着メッセージです。";
		if($mail_flg == 1){
			// 転送メール１へ
...
{$from}さんから新着画像メッセージが届いておりますのでお知らせ致します。
━［メッセージ内容］━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
※ご注意
このメールに返信しても該当パフォーマー様へメールは届きません。
返信される場合はサイト内のメッセージボックスよりご返信をお願いします。
...
{$from}さんから新着メッセージが届いておりますのでお知らせ致します。
━［メッセージ内容］━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
※メール内容の続きはサイト内のメッセージボックスをご確認ください。
