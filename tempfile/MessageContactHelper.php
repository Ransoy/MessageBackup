...
class MessageContactHelper{
	...
	function getAllFor($id, $type, $options=null){
			...
			case 1:
				if(is_array($options)){
					if($options['is_blocked'] == 1){
						...
						$this->db->select('contact.user_id,contact.performer_id,fave.contact_id,f.user_id,f.nick_name');
						$this->db->from('message_contact AS contact');
						...
						$this->db->select('contact.user_id,contact.performer_id,contact.is_from_chat,f.user_id,f.nick_name');
						$this->db->from('message_contact AS contact');
						$this->db->join('female_profile AS f','f.user_id = contact.performer_id','inner');
						$this->db->where("contact.user_id = '$id' ");
						if(count($options)>1){
					...
					$this->db->from('message_contact AS contact');
					 $this->db->join('message_contact_fave AS fave','fave.contact_id = contact.performer_id','left');
