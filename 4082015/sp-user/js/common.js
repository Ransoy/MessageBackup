...
$(function(){
	...
	*/
	...
	$('#simple-menu').sidr({
		...
		onOpen: function(){
			sidr_flg = 0;
			$('#icon_login img').attr('src', '/sp/image/icon/close_off.png');
			$('#icon_menu img').attr('src', '/sp/image/icon/close_off.png');
			$('#menu_close_area').toggle();
			$('#regist_footer_area').toggle();
			$('#simple-menu').css('background-color','black');
			$('#main').css({ position: '' /* 'fixed' */ }); // @see http://html-five.jp/38/
			$('#main').css({ top: '0' });
			var a = $('#sidr').width();
		...
		onClose: function() {
			...
			setTimeout(function(){
			$('#icon_login img').attr('src', '/sp/image/icon/login_off.png');
			$('#icon_menu img').attr('src', '/sp/image/icon/menu_off.png');
			$('#menu_close_area').toggle();
			$('#regist_footer_area').toggle();
			}, 300);
		}
	...
	$('#loginbtn').on('click',function(){
		...
		var boylogin = $.ajax({
			url : '/sp/ajax/boy_login_ajax.php?h=' + h,
			type : 'post',
			data : {
				'user_id':i,
				'password':p,
		...
		$.when(boylogin)
			.done(function(response){
				...
				if (location.search.slice(1).indexOf('is_app_access') > -1) {
					window.location.href = '/sp/chat/app/send_login_info_to_app.php';
				} else {
