...
class MessageInbox {
	...
	function displayAdmin($fromId, $read = 0, $keyword = '') {
		return $this->helper->getAdmin($fromId, $read, $keyword);
	}
	function displayAdmin2($fromId, $read = 0, $keyword = '') {
		return $this->helper->getAdmin2($fromId, $read, $keyword);
	}
		...
		if ($pages <= 6 ) {
			...
			if ($currentPage <= $mid) {
				if ($currentPage < 3) {
					...
					if ($currentPage-2 >= 2) {
						$pagination .="<li> <a href=\"javascript:void(0);\">1</a> </li>";
