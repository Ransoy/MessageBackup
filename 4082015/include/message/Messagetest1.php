	...
	function ManagePoints($type){
		...
		if($fromType == 1){
			...
			if(isSmartPhone()){
				...
				$totalMoney = $to['fp.MONEY'] + $arrPC[$type][$stat]+6;
			}
			...
			/* foreach ($arrTable as $key => $value){
				if(count($arrTable[$key]) > 3){
					$data = array(
							'point' => $arrTable['point'],
							'money' => $arrTable['money'],
							'money_old' => $arrTable['money_old'],
					);
				}else{
					$data = array('point' => $arrTable['point']);
				}
				
				$this->db->where('user_id','"'. $arrTable['id'] .'"');
				$result = $this->db->update($arrTable['table'], $data);
			} */
