...
class Message {
	...
	function uploadImage() {
		...
		if (!empty($image)) {
			...
			if (($ext == 'jpeg' || $ext == 'jpg' || $ext == 'JPEG')) {
				$fileProcess = $this->fixRotateImage($fileSource, $exif, $ext, $size);
			} else {
				$fileProcess = ($width > 440) ? $this->imageResize($fileSource, $ext, 440, 0) : $fileSource;
			}
	...
	function fixRotateImage($srcFile, $exif, $ext, $size) {
		$isImgModify = true;
