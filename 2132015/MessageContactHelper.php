...
class MessageContactHelper{
	...
	function getAllFor($id, $type, $options = null, $offset = 0, $limit = 0) {
        ...
        $this->db->sqlLogging('MessageContactHelper', 'getAllFor');
        return $this->db->get();       
	}
	...
	function getContactDetail($fromId, $toId, $toType, $fromType){
		...
		if($toType != 3){
			...
			$this->db->sqlLogging('MessageContactHelper', 'getContactDetail');
	...
	function getContactDetail2($fromId, $toId, $toType, $fromType) {
		...
		if($toType != 3){
			...
			$this->db->wherev2("$fromWhere = ? AND $toWhere = ?", array($fromId, $toId));
			$this->db->sqlLogging('MessageContactHelper', 'getContactDetail2');
			$result = $this->db->get_rowv2();
		} else {
			$this->db->select("ADMIN_ID as 'to_id', name as 'nick_name'");
			$this->db->from('admin');
	...
	function checkStatus($hash){
		...
		$this->db->sqlLogging('MessageContactHelper', 'checkStatus');
		$result = $this->db->get_rowv2();
	...
	function getUserId($hash, $type) {
		...
		$this->db->sqlLogging('MessageContactHelper', 'getUserId');
		$result = $this->db->get_rowv2();
		return $result;
	}
