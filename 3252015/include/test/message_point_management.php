...
$db->where('ID > 150 AND ID < 157 ORDER BY ID*1 ASC');
$malePoint = $db->get();

$db->select('ID, EXPLANATION, VALUE');
$db->from('point_setting');
$db->where('ID > 96 AND ID < 103 ORDER BY ID*1 ASC');
$femaleYen = $db->get();

$db->select('ID, EXPLANATION, VALUE');
$db->from('point_setting');
$db->where('ID > 102 AND ID < 109 ORDER BY ID*1 ASC');
$femalePoint = $db->get();

$db->select('ID, EXPLANATION, VALUE');
$db->from('point_setting');
$db->where('ID >= 109 AND ID <= 114 ORDER BY ID*1 ASC');
$imgMalePoint = $db->get();


$db->select('ID, EXPLANATION, VALUE');
$db->from('point_setting');
$db->where('ID >= 115 AND ID <= 120 ORDER BY ID*1 ASC');
$imgfemaleYen = $db->get();

$db->select('ID, EXPLANATION, VALUE');
$db->from('point_setting');
$db->where('ID >= 121 AND ID <= 126 ORDER BY ID*1 ASC');
$imgfemalepoint = $db->get();


$db->select('ID, EXPLANATION, VALUE');
$db->from('point_setting');
$db->where('ID >= 127 AND ID <= 132 ORDER BY ID*1 ASC');
$agentPcYen = $db->get();

$db->select('ID, EXPLANATION, VALUE');
$db->from('point_setting');
$db->where('ID >= 133 AND ID <= 138 ORDER BY ID*1 ASC');
$agentPcPoint = $db->get();

$db->select('ID, EXPLANATION, VALUE');
$db->from('point_setting');
$db->where('ID >= 139 AND ID <= 144 ORDER BY ID*1 ASC');
$agentSpYen = $db->get();

$db->select('ID, EXPLANATION, VALUE');
$db->from('point_setting');
$db->where('ID >= 145 AND ID <= 150 ORDER BY ID*1 ASC');
$agentSpPoint = $db->get();
