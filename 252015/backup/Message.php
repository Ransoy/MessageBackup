...
class Message {
	...
	function ManagePoints($type){
		...
		if($fromType == 1){
			...
			if($from['gold_flg'] == 1 && $from['stat'] !== 0){
				$stat = 2;
			}else{
				$stat = 0;
			}
			...
			if(isSmartPhone()){
				...
				$totalMoney = $to['money'] + $money['value'];
			}else{
			...
			$data = array(
					'point' => $toSendPoints,
					'point_old' => $from['point'],
				);
			$this->db->where('user_id','"'. $this->getFromId() .'"');
			$result = $this->db->update('male_point', $data);
			...
			$this->db->where('user_id','"'. $this->getToId() .'"');
			$result = $this->db->update('female_point', $data);
