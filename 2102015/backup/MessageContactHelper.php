...
class MessageContactHelper{
	...
	function getAllFor($id, $type, $options = null, $offset = 0, $limit = 0) {
        ...
        $this->db->where($where);        
        if(count($options)>1)
        	$value = preg_replace ('/[^\p{L}\p{N}]/u', ' ',  trim(addslashes($options['name'])));
	...
	function paginateContact($funcName, $countRow, $options, $limit , $type = 1){
		...
		if($totalRow > 1){
			...
			if($page > 1){
				$tmpl->assign('prev-href',$prev.$getview);
			...
			if ($totalRow <= 5){
				for ($counter = 1; $counter <= $totalRow; $counter++){
					if ($counter == $page){
						...
						$pagination .= "<li><a href='?page=$counter$getview'>$counter</a></li>";
					}
				}
			}else{
				$mid = ceil($totalRow/2);
				if($page <= $mid){
					if ($page <= 4) {
						...
						if (($page+2) > $rec_count) {
							...
							for ($counter = $totalRow - 11; $counter < ($totalRow-5); $counter++){
								if ($counter == $page){
									$pagination .= "<li class='current'><span>$counter</span></li>";
								}else{
									$pagination .= "<li><a href='?page=$counter$getview'>$counter</a></li>";
					...
					$pagination .= "<li><span>...</span></li>";
					$pagination .= "<li><a href='?page=$totalRow$getview'>$totalRow</a><li>";
					...
					if(($page+2) >= $totalRow){
						...
						for ($counter = $page - 4; $counter <= $totalRow; $counter++){
							if ($counter == $page){
								$pagination .= "<li class='current'><span>$counter</span><li>";
							}else{
								$pagination .= "<li><a href='?page=$counter$getview'>$counter</a></li>";
			...
			if ($page < $counter - 1){
				$tmpl->assign('next-href',$next.$getview);
	...
	function getContactDetail($fromId, $toId, $toType, $fromType){
		$result = array();
		if($toType != 3){
			if ($toType == 1) {
	...
	function getContactDetail2($fromId, $toId, $toType, $fromType) {
		$result = array();
		if($toType != 3){
			if ($toType == 1) {
				$toWhere = 'contact.user_id';
